...
//			}
			...
			if($this->controllerContext->getRequest()->hasArgument('sorting')) {
				$arguments['sorting'] = $this->controllerContext->getRequest()->getArgument('sorting');
			}
			...
			return $uriBuilder->reset()->setArguments(array($argumentPrefix => $arguments))->setSection($this->arguments['section'])->setAddQueryString(TRUE)->setAddQueryStringMethod($this->arguments['addQueryStringMethod'])->setArgumentsToBeExcludedFromQueryString($excludeArguments)->setFormat($this->arguments['format'])->build();
		}
	}
?>